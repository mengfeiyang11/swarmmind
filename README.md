...
5. **SSRF 防护**：使用 `ipaddress` 标准库检测内网 IP（含 IPv4/IPv6/映射地址），DNS 解析验证，禁止重定向跟随
6. **策略引擎**：默认拒绝（default-deny），安全的正则转义防止模式注入
...
| 工具 | 描述 | 权限 | 风险 |
|------|------|------|------|
| `call_api` | 调用 REST API（SSRF 防护） | standard | medium |
...
3. **网络安全**：SSRF 防护覆盖 IPv4/IPv6，DNS 解析验证，禁止重定向跟随
4. **命令执行安全**：命令白名单 + `shell=False`，防止 Shell 注入
5. **策略引擎**：默认拒绝策略，安全的模式匹配（正则元字符转义）
6. **审计日志**：线程安全的异步日志，记录所有 Agent 行为
